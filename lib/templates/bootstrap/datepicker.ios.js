...
class CollapsibleDatePickerIOS extends React.Component {
  ...
  render() {
    ...
    if (locals.config) {
      if (locals.value && locals.config.format) {
    ...
    return (
      <View>
        <TouchableOpacity style={[touchableStyle, , { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }]}
          onPress={() => {
          ...
          <Text style={[ dateValueStyle, formattedValue ? {} : { color: '#aaa' } ]}>
            { formattedValue || locals.placeholder || '' }
          </Text>

          <View style={{ justifyContent: 'center', opacity: .5, paddingRight: 12, marginTop: -2 }}>
            <Icon name="calendar" size={ 20 } color={ locals.hasError ? touchableStyle.borderColor : "#000" } />
          </View>
        </TouchableOpacity>
